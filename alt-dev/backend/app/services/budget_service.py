...
class BudgetService:
    ...
    def rate_for_category(self, category: str) -> float:
        """Return the effective per-token USD rate for a given category.
        - Defaults to config.cost_per_token_usd
        - For remote web LLM categories (prefix 'llm:web'), uses settings.BUDGET_LLM_WEB_COST_PER_TOKEN_USD if set, else falls back to default.
        """
        cfg = self.get_config()
        base = cfg.get("cost_per_token_usd", 0.0) or 0.0
        if category.startswith("llm:web"):
            remote = getattr(settings, "BUDGET_LLM_WEB_COST_PER_TOKEN_USD", 0.0) or 0.0
            return remote if remote > 0.0 else base
        return base
