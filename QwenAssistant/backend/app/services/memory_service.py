...
class MemoryService:
    ...
    # Tags
    def set_conversation_tags(self, conversation_id: str, tags: List[str]) -> Optional[DBConversation]:
        return self.db.set_conversation_tags(conversation_id, tags)

    def set_message_tags(self, message_id: str, tags: List[str]) -> Optional[DBMessage]:
        return self.db.set_message_tags(message_id, tags)

    def filter_conversations_by_tags(self, tags: List[str]) -> List[DBConversation]:
        return self.db.filter_conversations_by_tags(tags)
    ...
    def import_data(self, data: Dict, merge: bool = True) -> Dict:
        return self.db.import_data(data, merge)

    def get_conversation_stats(self, conversation_id: str) -> dict:
        """Return conversation statistics (messages count and token sum)."""
        return self.db.get_conversation_stats(conversation_id)
